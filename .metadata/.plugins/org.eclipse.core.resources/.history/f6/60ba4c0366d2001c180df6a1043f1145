...
<CENTER>
 ...
 <% 		// vector : 멀티스레드 환경에서 사용, 모든 메소드가 동기화 처리되어 있음.
  ...
  int priorpage = where-1;				// 이전페이지 : 현재 페이지 - 1
  int startrow=0;		// DataBase에서 Select한 레코드 시작 번호
