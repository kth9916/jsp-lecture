...
//	private LogonDBBean ( ) { super(); }; 
	...
	public void insertMember (LogonDataBean Member) {
		...
		try {
			...
			String sql = "insert into jmmember values (?, ?, ?, ?, ?, ?) "; 
...
//		SHA256 sha = SHA256.getInsatnce(); 
		...
		try {
			...
			String sql = "select passwd from jmmember where id = ? "; 
			psmt = con.prepareStatement(sql);
			psmt.setString(1, id);   
...
//			instance.close(); 	//객체 사용 종료. rs, smmt, psmt, con
	...
	public int confirmId (String id) {
		...
		try {
			String sql = "select id from jmmember where id = ?" ; 
			psmt = con.prepareStatement(sql);
			psmt.setString (1,id); 
			rs = psmt.executeQuery();
	...
	public LogonDataBean getMember (String id, String passwd) {
		...
		try {
			...
			String sql = "select * from jmmember where id = ?"; 
			psmt = con.prepareStatement(sql); 
			psmt.setString(1, id);
			rs = psmt.executeQuery(); 
			...
			if (rs.next()) {	// 해당 아이디가 DB에 존재한다.
				...
				if (orgPass.equals(dbpasswd)) {
					...
					//DTO (member)에서 들어온 값을 DB에 UPDATE 
					       sql = "update jmmember set name = ?, address = ?, tel= ? " ;  
						   sql += " where id = ?"; 
					psmt = con.prepareStatement(sql); 
