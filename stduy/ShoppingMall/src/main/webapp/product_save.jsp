 ...
 */
 ...
 int pos=0;
 		...
 		// DB에 값을 넣을 때 '도 처리해서 DB에 저장 되도록 설정
 ...
 try {
  ...
  sql = "select max(id) from  product where category= '"+cat+"'";
 		// DB의 id(상품의 고유 번호) : 기존의 category의 최대 값을 가져와서 +1 해서 상품 고유 번호 할당.	
  ...
  if (id==0) 	//가져온 레코드가 0 , category 컬럼의 상품이 존재하지 않는 경우 
   id=Integer.parseInt(cat+"00001");		// 11 + 00001
  else    
   id= id + 1 ;
 
  // 
  Enumeration files = multi.getFileNames();		// 파일 이름 2개을 한꺼번에 가져온다.
